...
import keras
from models import cnn_ngrams, cnn_lstm_sent, SiameseLSTM, ffnn, cnn_bilstm_attn_sent
...
def _setup_argparser():
    ...
    parser.add_argument("-m", "--model", action="store",
                        choices=["cnn_ngrams", "SiameseLSTM", "cnn_lstm",  "cnn_lstm_val", "cnn_bilstm_attn", "cnn_bilstm_attn_val","ffnn", "ffnn_val", "ffnn_val_test"],
                        default="cnn_ngrams",
                        type=str,
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        if args.model == "cnn_ngrams":
            ...
            model.train(save_path = out_trained_models,steps_train = 5000, steps_val = 1871)
            '''
            model = cnn_bilstm_attn_sent.Cnn_bilstm_attn_sentiment(train_generator = train_generator, validation_generator = validation_generator, conceptnet = True) ## conceptnet numberbatch
            ...
            total_steps_train = 5000 # increase to 50000 later
            ...
            model = cnn_lstm_sent.Cnn_lstm_sentiment(train_generator = gen_train, validation_generator = gen_val, conceptnet = True) ## set it to True/False for activate/deactivate commonsense
            ...
            model = cnn_lstm_sent.Cnn_lstm_sentiment(train_generator = gen_train, validation_generator = gen_val, conceptnet = False)
            model.train(save_path = out_trained_models, steps_train = total_steps_train, steps_val = total_steps_val)

        elif args.model == "cnn_bilstm_attn":
            
            print("CNN BILSTM ATTN training invoked")
            ...
            total_steps_train = 500
            total_steps_val = 500 #len(contexts_val)
            print("\nTOTAL POSSIBLE TRAIN STEP ARE: ", len(contexts_train_samp))
            print("TOTAL STEPS/EPOCH FOR TRAIN CHOSEN TO BE: ",total_steps_train)
            print("TOTAL STEPS/EPOCH FOR VALIDATION ARE: ",total_steps_val,"\n")
            ...
            gen_train = batch_iter_val_cnn_sentiment(contexts = contexts_train_samp, endings = endings_train_samp, binary_verifiers = binary_verifiers_train)
            gen_val = batch_iter_val_cnn_sentiment(contexts = contexts_val, endings = endings_val, binary_verifiers = binary_verifiers_val)
            print("GEn Train ", gen_train)

            model = cnn_bilstm_attn_sent.Cnn_bilstm_attn_sentiment(train_generator = gen_train, validation_generator = gen_val, conceptnet = True) ## conceptnet numberbatch
            model.train(save_path = out_trained_models, steps_train = total_steps_train, steps_val = total_steps_val)

        elif args.model == "cnn_bilstm_attn_val":
            
            print("CNN BILSTM ATTN  val training invoked")
            ...
            model = cnn_bilstm_attn_sent.Cnn_bilstm_attn_sentiment(train_generator = gen_train, validation_generator = gen_val)
            model.train(save_path = out_trained_models, steps_train = total_steps_train, steps_val = total_steps_val)
    ...
    if args.predict:
        ...
        if args.model == "SiameseLSTM":
        ...
        elif args.model == "cnn_lstm" or args.model == "cnn_lstm_val" or args.model == "cnn_bilstm_attn" or args.model == "cnn_bilstm_attn_val":
            ...
            Y_predict = model.predict_generator(test_generator, steps=2343) #10 #2343
