...
def embedding(docs, embedding_dim):
    ...
    '''
    # prepare tokenizer
    print("embedding doc length=",len(docs))
    #print (docs)
    ...
    f = open('numberbatch-en-17.06.txt','rb')
    print("Opening the Concept Numberbatch file")
    for line in f:
        ("Number batch file opened")
        values = line.split()
        word = values[0]
...
def pad_restructure_trainset(aug_data, ver_aug_data, train_context_notag):
    ...
    print("train context=",len(train_structured_context))
    # Saving datasets
